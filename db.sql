...
-- 뷰 shopdb.uv_memebertbl 구조 내보내기
DROP VIEW IF EXISTS `uv_memebertbl`;
-- VIEW 종속성 오류를 극복하기 위해 임시 테이블을 생성합니다.
CREATE TABLE `uv_memebertbl` (
	`memberName` CHAR(5) NOT NULL COLLATE 'utf8mb4_unicode_ci',
	`memberAddress` CHAR(20) NULL COLLATE 'utf8mb4_unicode_ci'
) ENGINE=MyISAM;

-- 트리거 shopdb.trg_dele 구조 내보내기
DROP TRIGGER IF EXISTS `trg_dele`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `trg_dele` AFTER DELETE ON `membertbl` FOR EACH ROW BEGIN
...
DROP TABLE IF EXISTS `uv_memebertbl`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `uv_memebertbl` AS SELECT memberName, memberAddress FROM membertbl ;
